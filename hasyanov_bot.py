...
TOKEN = "8203286904:AAH8pHnGacU5VqG4JIDEdqblPEIGZ7y0_gA"
...
# 📥 Отправка PDF — безопасная (только через callback_query)
...
# 📥 Отправка ДЗ + доп. файлы из подпапки (если есть)
...
# 🎛️ Выбор действия — 3 кнопки в ряд + эмодзи
async def on_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    
    # Определяем тип действия
    action_type = query.data.split("_")[1]  # "get", "check" или "notes"
    ...
    for i in range(0, len(NUMBERS), 3):
        ...
        for num in NUMBERS[i:i+3]:
            # Просто используем номер как текст кнопки (можно добавить эмодзи)
            if action_type == "get":
                button_text = f"{num}"  # ← ДЗ — книжка
            elif action_type == "check":
                button_text = f"{num}"  # ← Проверка — лупа
            elif action_type == "notes":
                button_text = f"{num}"  # ← Конспекты — бумага
            row.append(InlineKeyboardButton(button_text, callback_data=f"{query.data}_{num}"))
        keyboard.append(row)
...
async def on_get_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("action_get_"):
        ...
        # После отправки файла показываем главное меню
...
# 🔍 Проверить ДЗ - ИСПРАВЛЕННАЯ ВЕРСИЯ
async def on_check_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("action_check_"):
        ...
        # 🔹 Исправляем обработку 19_21
        if hw_num_str in ["1921", "19_21"]:  # Обрабатываем оба варианта
            hw_num = "19_21"
        else:
            try:
                hw_num = int(hw_num_str)
        ...
        # Добавляем кнопку "Отмена" в начало проверки
...
# ✍️ Ввод ответа — с полной нормализацией - ИСПРАВЛЕННАЯ ВЕРСИЯ
async def on_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔹 Нормализация номера ДЗ для поиска в словаре
    ...
    # 🔹 Нормализация: разбиваем по пробелам и собираем через один пробел
    ...
    # 🔹 ДЛЯ ОТЛАДКИ: выводим что сравниваем
    print(f"\n=== ДЕБАГ ДЗ {hw_key} ===")
    print(f"Задание #{task_num}")
    print(f"Пользователь ввел: '{user_input}' -> '{user_norm}'")
    print(f"Правильный ответ: '{correct_ans}' -> '{correct_norm}'")
    print(f"Сравнение: {user_norm == correct_norm}")
    print("=== КОНЕЦ ДЕБАГ ===\n")
    ...
    if next_task <= len(correct_answers):
        ...
        # 🔚 Проверка завершена — считаем результат
...
def main():
    ...
    if not os.path.exists(HW_DIR):
        logger.warning(f"Папка с ДЗ не найдена: {HW_DIR}")
        os.makedirs(HW_DIR, exist_ok=True)
    
    if not os.path.exists(NOTES_DIR):
        logger.warning(f"Папка с конспектами не найдена: {NOTES_DIR}")
